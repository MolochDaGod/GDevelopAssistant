 ...
 *   - If running inside GrudgeDotBox → proxied via /api/gruda-legion/babylon/*
...
function resolveBase() {
  // If we're on the same origin as GrudgeDotBox, use the proxy
  if (typeof window !== 'undefined') {
    try {
      const host = window.location.hostname;
      if (host === 'localhost' || host.includes('grudge-dot-box') || host.includes('grudge-studio')) {
        return PROXY_PREFIX;
      }
