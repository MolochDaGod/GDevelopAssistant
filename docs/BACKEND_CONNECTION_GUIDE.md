...
> This guide documents the working, production-verified pattern used by the grudgeDot launcher
...
```json
{
  "security": {
    "cors": {
      "origins": [
        "https://grudgedot.grudge-studio.com",
        "https://*.grudgedot-launcher.pages.dev",
...
The grudgeDot launcher includes a live probe dashboard at `/connections` that checks all 4
