        ...
        <Tabs defaultValue="servers" className="w-full">
          ...
          <TabsContent value="connections" className="space-y-4 mt-4">
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Card><CardHeader><CardTitle className="flex items-center gap-2">{allOnline ? <ShieldCheck className="h-5 w-5 text-emerald-500" /> : <ShieldAlert className="h-5 w-5 text-amber-500" />}Backend Status</CardTitle><CardDescription>Overall service health</CardDescription></CardHeader><CardContent className="space-y-2"><p className="text-sm"><span className="text-muted-foreground">Service:</span> <span className="font-medium">{healthQuery.data?.service || "GrudgeDotBox"}</span></p><p className="text-sm"><span className="text-muted-foreground">Backend URL:</span> <span className="font-medium break-all">{backendUrl}</span></p><p className="text-sm"><span className="text-muted-foreground">Health:</span>{" "}<Badge className={allOnline ? "bg-emerald-600 hover:bg-emerald-600 text-white" : "bg-amber-600 hover:bg-amber-600 text-white"}>{allOnline ? "Connected" : "Needs Attention"}</Badge></p></CardContent></Card>
            </div>
            <Card><CardHeader><CardTitle>AI Chat & Code Agents</CardTitle></CardHeader><CardContent><div className="flex flex-wrap gap-2"><Button onClick={() => navigate("/")}><MessageSquare className="mr-2 h-4 w-4" />Open AI Chat</Button><Button variant="secondary" onClick={() => navigate("/?tool=code-generate")}><Code2 className="mr-2 h-4 w-4" />Open Code Agent</Button><Button variant="outline" onClick={() => window.open("https://grudge-dot-box-git-main-grudgenexus.vercel.app/", "_blank")}><ExternalLink className="mr-2 h-4 w-4" />Open Deployed App</Button></div><Separator className="my-4" /><p className="text-xs text-muted-foreground">If probes show offline, verify Vercel environment values for `GRUDGE_GAME_API`, `GRUDGE_ACCOUNT_API`, `GRUDGE_ID_API`, `GRUDGE_LAUNCHER_API`, and `GRUDGE_AUTH_URL`.</p></CardContent></Card>
          </TabsContent>
